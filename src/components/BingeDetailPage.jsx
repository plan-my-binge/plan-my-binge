...
class BingeDetailPage extends Component<{ popularShows: any }> {
  ...
  render() {
    console.log("]]]]]]]]]]]]]]]]]]", this.props.location.data, this.props.match.params)

    let {recentShows, showError, showLoader, shows, match, location} = this.props;
    let bingeDetail = location.data || shows.find(x => x.pmbId == match.params.pmbId.split('-')[0]);

    return <Container>
      <h2>Find out how long does it take to watch every episodes of {bingeDetail.primaryTitle}, when you watch n hours a day</h2>
      ...
      <Content>
        ...
        {bingeDetail && recentShows.length !== 0 && <PopularShows shows={recentShows}/>}
      </Content>
    </Container>
  }
}
